...
def yearly_rain_route(app):
    ...
    def get_yearly_rain():
        ...
        print(f'[{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] Received request for yearly rain data between {start_time} and {end_time}')
...
def latest_yearly_rain_route(app):
        ...
        def get_latest_yearly_rain():
            ...
            entity_id_temperature = "gw1100a_v2_2_3_yearly_rain_rate"
            field = "value"
            print(f'[{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] Received request for latest yearly rain data')
            # Fetch data for temperature within the specified time range
