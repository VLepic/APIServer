...
def yearly_rain_route(app):
    ...
    def get_yearly_rain():
        ...
        entity_id_temperature = "gw1100a_v2_2_3_yearly_rain_rate"
        field = "value"

        # Retrieve start and stop time arguments from the request URL parameters
        start_time = request.args.get('start_time')
        end_time = request.args.get('end_time')
        logging.info(f'Received request for yearly rain data between {start_time} and {end_time}')
        ...
        try:
            ...
            logging.critical(f"Error fetching data for yearly rain: {InfluxDBReadError}")
...
def latest_yearly_rain_route(app):
        @app.route('/weather/yearly_rain/latest', methods=['GET'])
        def get_latest_yearly_rain():
            ...
            entity_id_temperature = "gw1100a_v2_2_3_yearly_rain_rate"
            field = "value"
            logging.info('Received request for latest yearly rain data')
            # Fetch data for temperature within the specified time range
            try:
                time_values, measurement_values = read_latest(INFLUXDB_URL, INFLUXDB_TOKEN, org, bucket, entity_id_temperature, field, "-620h")
            except Exception as InfluxDBReadError:
                logging.critical(f"Error fetching data for latest yearly rain: {InfluxDBReadError}")
                return jsonify({'500 Server error': 'An error occurred while fetching data from InfluxDB'}), 500
            # Convert datetime objects to ISO 8601 format
            time_values_iso = [time.isoformat() for time in time_values]
