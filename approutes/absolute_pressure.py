"""Route for absolute pressure measurements"""
...
def absolute_pressure_route(app):
    ...
    def get_absolute_pressure():
        ...
        entity_id_temperature = "gw1100a_v2_1_3_absolute_pressure"
        field = "value"

        # Retrieve start and stop time arguments from the request URL parameters
        start_time = request.args.get('start_time')
        end_time = request.args.get('end_time')
        logging.info(f'Received request for absolute pressure data between {start_time} and {end_time}')
        ...
        try:
            ...
            logging.critical(f"Error fetching data for absolute pressure: {InfluxDBReadError}")
...
def latest_absolute_pressure_route(app):
    ...
    def get_latest_absolute_pressure():
        ...
        entity_id_temperature = "gw1100a_v2_1_3_absolute_pressure"
        field = "value"
        logging.info('Received request for latest absolute pressure data')
        # Fetch data for temperature within the specified time range
        try:
            time_values, measurement_values = read_latest(INFLUXDB_URL, INFLUXDB_TOKEN, org, bucket, entity_id_temperature, field, "-1h")
        except Exception as InfluxDBReadError:
            logging.critical(f"Error fetching data for latest absolute pressure: {InfluxDBReadError}")
            return jsonify({'500 Server error': 'An error occurred while fetching data from InfluxDB'}), 500
        # Convert datetime objects to ISO 8601 format
        time_values_iso = [time.isoformat() for time in time_values]
