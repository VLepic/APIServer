...
def daily_rain_route(app):
    ...
    def get_daily_rain():
        ...
        print(f'[{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] Received request for daily rain data between {start_time} and {end_time}')
...
def latest_daily_rain_route(app):
    ...
    def get_latest_daily_rain():
        ...
        entity_id_temperature = "gw1100a_v2_2_3_daily_rain_rate"
        field = "value"
        print(f'[{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] Received request for latest daily rain data')
        # Fetch data for temperature within the specified time range
