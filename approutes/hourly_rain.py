...
def hourly_rain_route(app):
    ...
    def get_hourly_rain():
        ...
        print(f'[{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] Received request for hourly rain data between {start_time} and {end_time}')
...
def latest_hourly_rain_route(app):
    ...
    def get_latest_hourly_rain():
        ...
        entity_id_temperature = "gw1100a_v2_2_3_hourly_rain_rate"
        field = "value"
        print(f'[{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] Received request for latest hourly rain data')
