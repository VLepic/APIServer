...
def hourly_rain_route(app):
    ...
    def get_hourly_rain():
        ...
        logging.info(f'Received request for hourly rain data between {start_time} and {end_time}')
        ...
        try:
            ...
            logging.critical(f"Error fetching data for hourly rain rate: {InfluxDBReadError}")
...
def latest_hourly_rain_route(app):
    ...
    def get_latest_hourly_rain():
        ...
        entity_id_temperature = "gw1100a_v2_2_3_hourly_rain_rate"
        field = "value"
        logging.info('Received request for latest hourly rain data')

        # Fetch data for temperature within the specified time range
        try:
            time_values, measurement_values = read(INFLUXDB_URL, INFLUXDB_TOKEN, org, bucket, entity_id_temperature, field, "-620h")
        except Exception as InfluxDBReadError:
            logging.critical(f"Error fetching data for latest hourly rain rate: {InfluxDBReadError}")
            return jsonify({'500 Server error': 'An error occurred while fetching data from InfluxDB'}), 500
        # Convert datetime objects to ISO 8601 format
        time_values_iso = [time.isoformat() for time in time_values]
        return jsonify({'time_values': time_values_iso, 'measurement_values': measurement_values}), 200